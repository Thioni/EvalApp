...
<body class="bg-secondary">  
  ...
  <div class="container-fluid">
    ...
    <section class="row m-0 justify-content-center text-center">
      ...
      <div class="col-2 bg-light fs-3 text-uppercase text-center"><?= $contact->getFirst_name()." ".$contact->getLast_name(); ?></div>
    ...
    <section class="row m-0 justify-content-center text-center mb-1">
      ...
      <div class="col-1 bg-light text-center">
        <?php $convertTimestamp = strtotime($contact->getBirthdate());
        echo $frenchFormat = date("d-m-Y", $convertTimestamp); ?>
      </div>

      <div class="col-1 bg-light text-center">
        <?php foreach ($countries as $country): ?>
          <?= $contact->getNationality() === $country->getId() ? $country->getLocation() : ""; ?>
        <?php endforeach; ?>
      </div >

      <div class="col-2 bg-light text-center">
        <?php foreach ($codenames as $codename): ?>
            <?= $contact->getCodename() === $codename->getId() ? $codename->getAlias() : ""; ?>
          <?php endforeach; ?>
      </div >
    ...
    <div class="col-1 offset-3 mb-3">    
      <a href="../Controller/deleteContact.php?id=<?= $contact->getId() ?>" class="btn btn-danger">Supprimer</a>
    </div>
